...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"libs/mobilebone.js":[function(require,module,exports) {
var define;
...
(function (root, factory) {
  ...
  var store = {}; // Create local references to array methods we'll want to use later.

  var array = [];
  var slice = array.slice; // Is it a id selector

  var isSimple = /^#?\w+(?:[\-_]\w+)*$/i; // Is it webkit

  var isWebkit = "WebkitAppearance" in document.documentElement.style || typeof document.webkitHidden != "undefined"; // Is it suppory history API
  ...
  **/
  ...
  Mobilebone.transition = function (pageInto, pageOut, back, options) {
    ...
    pageOut = pageOut || null, back = back || false, options = options || {}; // defaults parameters
    ...
    var defaults = {
      ...
      callback: this.callback
        ...
        params = function params(element) {
      if (!element || !element.getAttribute) return {};

      var _params = {},
          _dataparams = _queryToObject(element.getAttribute("data-params") || ""); // rules as follow:
    ...
    var paramsOut = params(pageOut),
        paramsIn = params(pageInto);

    if (pageOut != null && pageOut.classList) {
      // weather prevent transition
      var preventOut = paramsOut.preventdefault,
          isPreventOut = false;
    ...
    if (pageInto != null && pageInto.classList) {
      // weather prevent transition
      var preventInto = paramsIn.preventdefault,
          isPreventInto = false;
    ...
    var funAnimationCall = function funAnimationCall(page, data) {
      if (page.flagAniBind == true) return; // do callback when animation start/end
      ...
      ["animationstart", "animationend"].forEach(function (animationkey, index) {
        ...
        var animateEventName = isWebkit ? webkitkey : animationkey; // if it's the out element, hide it when 'animationend'

        if (index) {
          page.addEventListener(animateEventName, function () {
            if (this.classList.contains("in") == false) {
              this.style.display = "none"; // add on v2.5.5
              // move here on v2.5.8
    ...
    if (pageOut != null && pageOut.classList) {
      ...
      if (isPreventOut != true) {
        pageOut.classList.add(paramsOut.form); // reflow

        pageOut.offsetWidth = pageOut.offsetWidth; // go, go, go

        pageOut.style.display = "block";
        pageOut.classList.add("out");
        pageOut.classList.remove("in"); // if reverse direction

        pageOut.classList[back ? "add" : "remove"]("reverse"); // add on v2.5.5

        pageOut.removeSelf = pageOut.removeSelf || null; // set animation callback for 'pageInto'
        // for issues #153

        funAnimationCall(pageOut, paramsOut); // do fallback every time
    ...
    if (pageInto != null && pageInto.classList) {
      ...
      var title = paramsIn.title,
          ...
          firstPage = document.querySelector("." + this.classPage); // do title change
      ...
      var hashid = options.id || pageInto.id; // v2.7.0 change rule -> don't auto delete pages
      ...
      */
      ...
      pageInto.offsetWidth = pageInto.offsetWidth; // go~ as normal

      pageInto.style.display = "block";
      pageInto.classList.remove("out");
      pageInto.classList.add("in"); // if reverse direction

      pageInto.classList[back ? "add" : "remove"]("reverse"); // do callback when come in first time

      var onpagefirstinto = paramsIn.onpagefirstinto; // first judge change to pageInto store
      ...
      funAnimationCall(pageInto, paramsIn); // history
      ...
      if (supportHistory && this.pushStateEnabled && options.history !== false && urlPush // hash should be different
      // can fix issues #79, #87 maybe
      && urlPushReplace != location.hash) {
        // don't trigger 'popstate' events
        history.popstate = false; // if only pageIn, use 'replaceState'
      ...
      if (!store[pageid]) {
        store[pageid] = pageInto; // when we back/prev, we need to get true
        // comment on v2.7.0
        // if (hashid !== pageid) {
  ...
  Mobilebone.remove = function (domOrId) {
    ...
    if (elePage && elePage.parentElement) {
      elePage.parentElement.removeChild(elePage); // remove store
  ...
  Mobilebone.getCleanUrl = function (trigger, url, params) {
    ...
    if (trigger) {
      if (trigger.nodeType == 1) {
        // form element
        if (trigger.action) {
          href = trigger.getAttribute("action"); // add on v2.4.1
          ...
          if (trigger.method && trigger.method.toUpperCase() == "POST") {
            ...
            formdata = {}; // simple serialize from Mobilebone
          ...
          formdata = trigger.getAttribute("data-formdata") || trigger.getAttribute("data-data") || ""; // v2.6.1 for #107
          // remember container when refresh

          var strContainer = "container",
              attrContainer = trigger.getAttribute("data-" + strContainer);
  ...
  **/
  ...
  Mobilebone.createPage = function (domHtml, eleOrObj, options) {
    var response = null,
        ...
        isreload = null; // 'eleOrObj' can '.page element', or 'a element', or 'options'
    // basically, options = ajax options, of course, u can custom it!

    if (!domHtml) return;

    if (typeof options == "undefined" && _typeof(eleOrObj) == "object") {
      options = eleOrObj;
    }

    options = options || {}; // 'options' that 'Mobilebone.transition()' needs

    var optionsTransition = {}; // get page-title from eleOrObj or options
    ...
    if (eleOrObj) {
      if (eleOrObj.nodeType == 1) {
        ...
        classPageInside = eleOrObj.getAttribute("data-classpage"); // pass element as target params, add on v2.3.0

        optionsTransition.target = eleOrObj; // v2.4.4 is_root → isreload
        ...
        optionsTransition.back = eleOrObj.getAttribute("data-rel") == "back"; // v2.6.0 history
        ...
        optionsTransition.target = eleOrObj.target; // v2.5.2
        // is back? for issues #128
    ...
    var eleCurrentPage = (classPage == classPageInside ? container : document).querySelector(".in." + classPage); // get create page (will be into) according to 'domHtml'
    ...
    var eleCreateTitle = create.getElementsByTagName("title")[0]; // get the page element
    ...
    } else if (eleCreateTitle && eleCreateTitle.innerText) {
      // the judge behind '&&' for issues #144
      eleCreatePage.setAttribute("data-title", eleCreateTitle.innerText);
    } // do transition


    optionsTransition.response = response || domHtml;
    optionsTransition.id = this.getCleanUrl(eleOrObj) || eleCreatePage.id || options.id || options.url || "unique" + Date.now(); // 'if' statement below added on v2.0.0
    ...
    container = container || document.body; // remove page if have same id when optionsTransition.remove == true
  ...
  **/
  ...
  Mobilebone.getFunction = function (keys) {
    if (typeof keys != "string") return; // eg. 'globalObject.functionName'

    var fun = Mobilebone.rootTransition,
        arrKey = keys.split(".");
  ...
  **/
  ...
  Mobilebone.ajax = function (aOrFormOrObj) {
    if (!aOrFormOrObj) return; // default params
    ...
    var params = {},
        eleMask = null,
        formData = null; // classname of mask

    var classMask = this.classMask; // if 'aOrFormOrObj' is a element, we should turn it to options-object

    var paramsFromTrigger = {},
        attrMask;

    if (aOrFormOrObj.nodeType == 1) {
      paramsFromTrigger = _queryToObject(aOrFormOrObj.getAttribute("data-params") || ""); // get params
      ...
      var queryFromUrl = _queryToObject(params.url.split('?')[1]); // v2.7.4 fix params may ingore problem
      ...
      params.query = paramsFromTrigger; // store target

      params.target = aOrFormOrObj; // v2.5.2
      // is back? for issues #128
      ...
      if (tagName == "form") {
        ...
        if (container && classPageInside && classPageInside != Mobilebone.classPage) {
          ...
          params.history = false; // title do not change
      ...
      if (attrMask == "true" || attrMask == "") {
        ...
        params.url = this.getCleanUrl(null, params.url, params.data); // here params.title will become page title;

        params.title = aOrFormOrObj.title; // v2.5.2
        // is back? for issues #128
        // when history.back()

        params.back = aOrFormOrObj.back; // v2.6.1

        params.container = aOrFormOrObj.container; // v2.7.4
    ...
    xhr.onload = function () {
      ...
      if (xhr.status == 200) {
        if (params.dataType == "json" || params.dataType == "JSON") {
          ...
          // ajax send by url
          // no history hush
          params.history = false; // I don't remember why add 'params.remove = false' here,
          ...
          response = xhr.response; // 'response' is string
      ...
      params.complete.call(params, xhr, xhr.status); // hide loading
    ...
    xhr.onerror = function (e) {
      ...
      params.error.call(params, xhr, xhr.status); // hide loading
  ...
  **/
  ...
  Mobilebone.submit = function (form) {
    ...
    form.addEventListener("submit", function (event) {
      // prevent detect
      var attrPrevent = this.getAttribute("data-preventdefault"); // get 'preventDefault' function
  ...
  **/
  Mobilebone.init = function () {
    ...
    if ($ && $.fn && $.fn.tap && 'ontouchstart' in window == true) {
      ...
      $(document).tap(this.handleTapEvent); // zepto tap event.preventDefault can't prevent default click-events
      ...
      document.addEventListener("click", function (event) {
        ...
        var ajax = target.getAttribute("data-ajax"),
            href = target.href; // if not ajax request
  ...
  **/
  ...
  Mobilebone.handleTapEvent = function (event) {
    ...
    */
    var target = null; // you can pass target as params directly
    ...
    var attrPrevent = target.getAttribute("data-preventdefault") || _queryToObject(target.getAttribute("data-params") || "").preventdefault; // get 'preventDefault' function
    ...
    if (container && classPageInside && classPageInside != Mobilebone.classPage) {
      selfPage = container.querySelector(".in." + classPageInside) || container.querySelector(classPageInside); // if (selfPage == null) return false;
    ...
    var capture = Mobilebone.captureLink == true; // get rel

    var rel = target.getAttribute("data-rel"); // if back
    ...
    var external = rel == "external"; // if the 'href' is not legal, return
    // include:
    // 1. undefined
    ...
    var attrHref = target.getAttribute("href"); // judge that if it's a ajax request

    if (/^#/.test(attrHref) == true) {
      // hash slide
      var hashTargetPage = href.split("#")[1];
      var idTargetPage = hashTargetPage.split("?")[0];
      var queryTargetPage = hashTargetPage.split("?")[1]; // add on v2.7.4
      ...
      var cleanUrl = Mobilebone.getCleanUrl(target); // add on v2.7.4

      options.query = _queryToObject(cleanUrl.split("?")[1]); // if has loaded and the value of 'data-reload' is not 'true'
  ...
  **/
  ...
  var _queryToObject = function _queryToObject(string) {
    var obj = {};
  ...
  **/
  ...
  window.addEventListener("popstate", function () {
    ...
    var hash = location.hash.replace("#&", "#").replace(/^#/, ""); // add on v2.7.4

    var key = hash.split('?')[0];
    var pageIn = null; // add on v2.6.1
    ...
    if (hash == "") {
      ...
      // add on v2.7.4
      pageIn = store[key] || store[hash]; // add on v2.6.1
    ...
    if (pageIn && pageIn == pageOut || Mobilebone.pushStateEnabled == false) return; // hash ↔ id
...
},{}],"C:/Users/LucyS/AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["C:/Users/LucyS/AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","libs/mobilebone.js"], null)
//# sourceMappingURL=/mobilebone.c5f78b34.js.map